...
import { mobileVerifyAction } from './../../../Redux/actions/EmployeeLoginActions';
...
export default function MobileVerification(props) {
    ...
    const signUpData = useSelector((state) => state.employeeMobileVerify);
    const { mobileVerify } = signUpData;
    ...
    useEffect(() => {
        if (mobileVerify) {
            ...
            if (mobileVerify?.success) {
                setLoading(false)
                var encodedStringBtoA = window.btoa(mobileVerify.data.token);
    ...
    const verifyOtp = async (values, formikHelpers) => {
        ...
        setLoading(true)
        dispatch(mobileVerifyAction({ userObject: boday }))
    }
